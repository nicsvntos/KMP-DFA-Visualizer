<script lang="ts">
  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="rounded-xl border border-slate-700 bg-slate-900 p-4 flex flex-col gap-4">
      <p class="text-base font-medium text-slate-300">Build a Rule</p>
      ...
      <div class="flex gap-2">
        {#each (['SINGLE', 'AND', 'OR'] as const) as op}
          <button
            onclick={() => (operator = op)}
            class="flex-1 py-1.5 text-sm font-medium rounded-md border transition-colors
                   {operator === op
      ...
      <div class="flex flex-col gap-3">
        <div class="flex items-center gap-2">
          <button
            ...
            class="shrink-0 px-2 py-1.5 text-sm rounded-md border transition-colors font-mono
                   {negateA
        ...
        {#if operator !== 'SINGLE'}
          <div class="flex items-center gap-2">
            <span class="shrink-0 px-2 py-1.5 text-sm rounded-md bg-indigo-900 border border-indigo-700 text-indigo-300 font-mono">
              {operator}
            </span>
            <button
    ...
    {#if hasRules}
        ...
        {#each rules as rule, i}
          <div class="flex items-center justify-between rounded-lg border border-slate-700
            ...
            <div class="flex items-center gap-2 min-w-0">
              <span class="text-sm text-slate-500 shrink-0">{rule.label}</span>
            ...
            <div class="flex items-center gap-2 shrink-0">
              {#if validationResults[i]}
                <span class="text-sm font-bold {statusColor(validationResults[i].result)}">
              ...
              <button
                ...
                class="text-sm px-2 py-0.5 rounded border border-slate-700
      ...
      {#if consistencyReport}
        <div class="rounded-lg border p-3
          ...
          <p class="text-sm mt-1 {consistencyReport.status === 'consistent' ? 'text-green-300' : 'text-red-300'}">
      ...
      {#if tables[activeTableIndex]}
        ...
        <div class="rounded-xl border border-slate-700 bg-slate-900 p-4 flex flex-col gap-3">
          <div class="flex items-center justify-between flex-wrap gap-2">
            <div>
              <p class="text-base font-medium text-white">{table.rule.label} — Truth Table</p>
              <p class="text-sm font-mono text-indigo-300 mt-0.5">{exprToString(table.rule.expr)}</p>
            </div>
            <div class="flex gap-2">
              {#if table.isTautology}
                <span class="text-sm px-2 py-0.5 rounded bg-green-900 border border-green-700 text-green-300">Tautology</span>
              {/if}
              {#if table.isContradiction}
                <span class="text-sm px-2 py-0.5 rounded bg-red-900 border border-red-700 text-red-300">Contradiction</span>
              {/if}
              {#if table.isSatisfied && !table.isTautology}
                <span class="text-sm px-2 py-0.5 rounded bg-indigo-900 border border-indigo-700 text-indigo-300">Satisfiable</span>
              {/if}
            </div>
          </div>
      ...
      {#if validationResults.length > 0}
        <div class="flex flex-col gap-2">
          <p class="text-base font-medium text-slate-300">Validation Results</p>
          {#each validationResults as vr}
            <div class="rounded-lg border px-3 py-2 flex flex-col gap-1 {statusBg(vr.result)}">
              <div class="flex items-center justify-between">
                <span class="text-sm font-semibold">{vr.rule.label}</span>
                <span class="text-sm font-bold">
                  {vr.verdict === 'accepted' ? '✓ Accepted' : '✗ Rejected'}
                </span>
              </div>
              <p class="text-sm font-mono opacity-80">{exprToString(vr.rule.expr)}</p>
              <div class="flex flex-wrap gap-2 mt-1">
                {#each Object.entries(vr.assignments) as [p, found]}
